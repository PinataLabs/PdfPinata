...
PdfPinata is a .NET library for processing PDF file.
You create PDF pages using drawing routines known from GDI+.
Almost anything that can be done with GDI+ will also work with PdfPinata.
Keep in mind it does no longer depend on GDI+. Imaging and font metadata come from a backend package you choose - `PdfPinata.Skia` ([SkiaSharp](https://github.com/mono/SkiaSharp), MIT) or `PdfPinata.ImageSharp` ([ImageSharp](https://github.com/SixLabors/ImageSharp), Apache-2.0) - which you register at startup. See the [README](https://github.com/PinataLabs/PdfPinata#backends) for details.
Only basic text layout is supported by PdfPinata, and page breaks are not created automatically.
...
Both PdfPinata and PinataLayout provide a lot of `AddXxx` functions.
