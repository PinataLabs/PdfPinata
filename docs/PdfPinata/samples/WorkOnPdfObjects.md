...
> `dotnet run --project src/SampleApp -- run -e Inspect`
> `dotnet run --project src/SampleApp -- run -e Navigation`
>
...
This sample shows how to deal with PDF objects that are not (yet) covered by specialized PdfPinata classes (as an example it adds an OpenAction to an existing PDF file).

PDF documents are based internally on objects like dictionaries, arrays, streams etc. This sample shows how to work directly on these underlying PDF objects. Use this functionality to achieve PDF features that are not yet implemented in PdfPinata.
...
// The current version of PdfPinata doesn't support the concept of
...
// Using PdfPinata we never deal with object numbers. We simply put the
// objects together and the PdfPinata framework does the rest.
```

Other objects not covered by PdfPinata can also be added this way.

Using PdfPinata we never deal with object numbers. We simply put the objects together and the PdfPinata framework does the rest.
