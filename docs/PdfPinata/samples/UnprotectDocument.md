...
> `dotnet run --project src/SampleApp -- run -e Protect`
>
...
Note: that we will not explain nor give any tips how to crack a protected document with PdfPinata.
...
// was the user or the owner password. In both cases PdfPinata provides full
// access to the PDF document. It is up to the programmer who uses PdfPinata
// to honor the access rights. PdfPinata doesn't try to protect the document
