...
Covered by `src/PdfPinata.Test/IO/TextExtractionTests.cs` and `TaggedTextExtractionTests.cs`.
...
run time. Index it. The pre-existing `src/PdfPinata.Test/Helpers/TextOperators.cs` had already worked
