...
**Prior art to follow rather than reinvent.** `src/PdfPinata.Test/IO/` holds the reader and mode
tests; `PageResizeTests` already exercises a document opened for modification and is a model for
arranging one. `src/PdfPinata.Test/Helpers/PdfHelper.cs` builds documents to open. `RawPdf.cs` builds
...
`src/PdfPinata.Test/Demos/DemoSmokeTests.cs` fails the build when a demo throws or changes its page
